...
def remove_newline_in_pair_tag (content):
    """remove the \n inside one pair tag"""
...
def remove_special_char (content):
    """hard to handle, ignore this case"""
    r = re.compile (r'''(<[^<>]*?)(["'])\s*javascript\s*:[^<>]+([<>]).*?\2([^>]+>)''',re.I|re.M|re.S)
    s = r.sub(r'\1\2 \2 \4',content)
    return s
...
def extract (content):
    ...
    content = remove_empty_line(remove_newline_in_pair_tag (content))
